...
#              deliberately avoiding the rule's zoom.us exclusion. The network
#              sensor (Packetbeat / network_traffic TLS parser) reads the SNI from
#              the ClientHello and the network_traffic.tls ingest pipeline copies
#              it into destination.domain, which is the field the detection rule
#              matches with RLIKE.
#
#              The flow is emitted as raw IP/TCP packets to a NON-LOCAL,
#              documentation-range destination (TEST-NET-3, 203.0.113.0/24) so the
#              packets egress the physical NIC where the sensor captures them. A
#              same-host connection to get_host_ip() would route over loopback
#              (lo), which the sensor does not observe, so raw sockets to a
#              non-local destination are required. The source is spoofed; the local
#              kernel holds no TCP state and any reply goes nowhere, as expected.
#
#              Flow: SYN -> PSH+ACK (TLS ClientHello with the DGA SNI) -> RST.
#              tls.established is false (no ServerHello), but the SNI - and
#              therefore destination.domain - is populated from the ClientHello.
#
...
TLS_PORT = 443
SPOOFED_SOURCE_IP = "10.10.10.21"
EXTERNAL_DEST_IP = "203.0.113.20"
...
def main() -> None:
    """Emit a raw TLS ClientHello whose SNI matches the FIN7 DGA domain pattern."""
    if os.geteuid() != 0:
    ...
    client_hello = _client_hello(DGA_SNI)

    sock = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_TCP)
    sock.setsockopt(socket.IPPROTO_IP, socket.IP_HDRINCL, 1)

    log.info(
        "Forging TLS ClientHello with SNI %s -> %s:%d (FIN7 DGA C2 emulation)",
        DGA_SNI, EXTERNAL_DEST_IP, TLS_PORT,
    )
    try:
        # SYN opens the flow.
        _ = sock.sendto(
        ...
        time.sleep(0.05)
        # PSH+ACK carries the ClientHello with the DGA SNI.
        _ = sock.sendto(
            _build_raw_packet(
        ...
        log.info("Forged FIN7 DGA SNI ClientHello emitted (destination.domain=%s expected)", DGA_SNI)
    except OSError as e:
