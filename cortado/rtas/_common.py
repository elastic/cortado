...
def execute_command(  # noqa
    ...
    **kwargs,
    ...
    if isinstance(command, (list, tuple)):
        ...
        sys.stderr.write("Deprecation warning! Switch arguments to a list for _common.execute()\n\n")
...
def link_file(source, target):
    ...
    execute_command(["ln", "-s", source, target])
...
def clear_web_cache():
    ...
    execute_command(["RunDll32.exe", "InetCpl.cpl,", "ClearMyTracksByProcess", "8"], hide_log=True)
    time.sleep(1)


def serve_web(ip=None, port=None, directory=None):
...
def find_remote_host():
    ...
    _, stdout = execute_command("net view", hide_log=True)
    hosts = re.findall(r"\\\\([\w\d\._-]+)", stdout)

    # _, current_user = execute_command("whoami", hide_log=True)
    pending = {}
...
def resolve_to_ipv4(hostname: str) -> str:
    ...
    code, output = execute_command(["ping", hostname, "-4", "-n", 1], hide_log=True)
    if code != 0:
        return None
...
def enable_logon_auditing(host="localhost", verbose=True, sleep=2):
    ...
    enable = execute_command(command)
