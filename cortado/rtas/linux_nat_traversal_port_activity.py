...
#              internal (RFC1918) source IP to a public (external) destination IP.
#
#              The updated rule no longer matches any UDP traffic to port 4500; it
#              requires the NAT-T signature specifically:
...
def main() -> None:
    """Emit a raw UDP datagram with src.port and dst.port both 4500, internal source to public dest."""
    if os.geteuid() != 0:
        log.error("Raw socket privileges required (run as root or with CAP_NET_RAW)")
        return

    packet = _build_udp_packet(
        INTERNAL_SOURCE_IP, PUBLIC_DESTINATION_IP, NAT_T_PORT, NAT_T_PORT, _NAT_T_PAYLOAD
    )

    log.info(
        "Sending NAT-T datagram: %s:%d -> %s:%d (IPSEC NAT Traversal emulation)",
        INTERNAL_SOURCE_IP, NAT_T_PORT, PUBLIC_DESTINATION_IP, NAT_T_PORT,
    )
